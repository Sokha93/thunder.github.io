(() => {
    ...
    let mx           = 0; // for saving coordinates
    let my           = 0; // for saving coordinates
    let toggle       = 0; //for the switch the number wich will move a certain circle
    let circles      = [];
    let circlesCount = 2;
    const stepLength = 2;
    const maxOffset  = 6;
    ...
    class Circle{ //create circles
        ...
        draw(x, y) { //
            ...
            ctx.fillStyle   = 'white'; //cvet zalivki
            ctx.strokeStyle = 'red';//cvet contura

            ctx.beginPath(); //otkroem put
            ctx.arc(this.x, this.y, 6, 0, TWO_PI); //risuem tochku, x, y, rad-6, nachalniy ugol- 0, i konechniy ugol 2p;
            ctx.closePath(); //zakrivaem
            ctx.fill(); // zalivaem


            ctx.beginPath(); //narisovka contura
